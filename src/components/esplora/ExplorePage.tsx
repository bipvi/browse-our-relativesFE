...
import { ChevronRight, AlertCircle, TreePine, ArrowLeft } from 'lucide-react'
...
// ─── Selected item card ───────────────────────────────────────────────────────
function SelectedCard({ item }: { item: any }) {
  const { favourites, addFavourite, removeFavourite } = useUserStore()
  const isFav = Array.isArray(favourites) && favourites.some(f => f?.id === item?.id)
  const [detailOpen, setDetailOpen] = useState(false)
  const tipo = item?.tipo?.toLowerCase()
      ...
      <div className={cn(
        ...
        <div className="p-5 space-y-4">
          ...
          <Button
            ...
            className="w-full bg-myP text-myS font-semibold hover:bg-myP/80 rounded-xl h-9 text-sm"
          >
            Dettaglio completo
          </Button>
        </div>
...
export default function ExplorePage() {
  ...
  const navigateTo = useCallback(async (partialItem: any) => {
    ...
    try {
      ...
      window.history.replaceState(null, '', `/${full.id}`)
  ...
  useEffect(() => {
    if (!params?.itemId || !token) return
    setLoading(true)
    setError('')
    fetch(`${API}/item/${params.itemId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => { if (!r.ok) throw new Error(`${r.status}`); return r.json() })
      .then(({ tipo, id }) => fetchFullItem(tipo, id))
      .then(full => selectItem(full))
    ...
    <div className="w-full px-4 sm:px-8 lg:px-16 py-8 min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap min-w-0 flex-1">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-1.5 text-txt/40 hover:text-txt transition-colors text-sm shrink-0 mr-1"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <TreePine className="h-4 w-4 text-myP/60 shrink-0" />
      ...
      {error && (
        <div className="flex items-center gap-2 text-amber-400 bg-amber-400/10 border border-amber-400/20 rounded-xl px-4 py-3 mb-6">
          ...
          <span className="text-sm">Errore nel caricamento ({error}). Riprova.</span>
        </div>
      )}
          ...
          {loading || childrenLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-52 rounded-2xl border border-dashed border-white/10 gap-3">
              ...
              <p className="text-sm text-txt/30 text-center">
                ...
                <button onClick={() => router.push('/curiosone')} className="text-myP/60 hover:text-myP transition-colors mt-1 inline-block">
                  Esplora il Curiosone →
                </button>
              </p>
            </div>
          ) : children.length === 0 ? (
