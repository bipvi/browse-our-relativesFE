...
export default function CuriosoneHeading() {
  ...
  const fetchSpecie = async (setter: (d: any) => void) => {
    try {
  ...
  useEffect(() => {
    if (token) {
      setIsLoading(true)
      Promise.all([fetchSpecie(setItem), fetchSpecie(setOther)]).finally(() => setIsLoading(false))
    }
  }, [token])
  ...
  const handleRefresh = async () => {
    setIsLoading(true)
    await Promise.all([fetchSpecie(setItem), fetchSpecie(setOther)])
    setIsLoading(false)
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-8">
        ...
        <div className="md:col-span-3 lg:col-span-4 space-y-6">
          ...
          {!isLoading && taxonomy.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-txt/40 uppercase tracking-widest mb-3">
                Gerarchia tassonomica
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4">
                {taxonomy.map((sub: any, i: number) => (
                  <MicroCard key={i} item={sub} />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="hidden md:flex md:flex-col gap-4 md:col-span-1">
          <p className="text-xs font-semibold text-txt/40 uppercase tracking-widest">Altre specie</p>
          {isLoading
            ? Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-48 rounded-xl" />)
            : otherItems.map((sub: any, i: number) => (
                <MicroCard key={i} item={sub} classe={i > 1 ? 'hideLastCard' : ''} />
              ))
          }
        </div>
      </div>
