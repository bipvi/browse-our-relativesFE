...
export default function MyFooter() {
  ...
  return (
    <footer className="relative w-full mt-16" style={{ backgroundColor: 'rgba(0,18,20,0.95)' }}>
      {/* Top border accent */}
      <div className="h-px w-full bg-gradient-to-r from-transparent via-myP/40 to-transparent" />

      <div className="mx-auto max-w-7xl px-6 sm:px-8 py-14">
        <div className="grid grid-cols-1 gap-12 nav:grid-cols-2">
            ...
            <div className="flex gap-3 mt-1">
              {SOCIALS.map(({ label, href, icon }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 text-txt/40 hover:text-myP hover:bg-myP/10 border border-white/5 hover:border-myP/20 transition-all"
                >
          ...
          <div className="grid grid-cols-3 gap-8">
            {LINKS.map(({ title, items }) => (
              <div key={title}>
                <p className="text-xs font-semibold text-txt/30 uppercase tracking-widest mb-4">{title}</p>
                <ul className="space-y-2.5">
                  {items.map(link => (
                    <li key={link}>
                      <a href="#" className="text-sm text-txt/50 hover:text-myP transition-colors">
                        {link}
                      </a>
                    </li>
                  ))}
        ...
        <div className="mt-12 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-5">
            {['Privacy', 'Termini', 'Cookie'].map(label => (
              <a key={label} href="#" className="text-xs text-txt/25 hover:text-txt/60 transition-colors">
                {label}
              </a>
            ))}
          </div>
