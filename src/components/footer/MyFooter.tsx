      ...
      <div className="mx-auto w-full max-w-7xl px-8">
        <div className="grid grid-cols-1 justify-between gap-4 nav:grid-cols-2">
          <div className="flex items-center h-min justify-center gap-4">
            <img src="/favicon.svg" className="size-16 logo-shadow" alt="logo" />
            <h5 className="text-xl font-semibold text-shadow">Browse our relatives</h5>
          </div>
          <div className="grid grid-cols-3 justify-between gap-4">
            {LINKS.map(({ title, items }) => (
              <ul key={title}>
                <p className="mb-3 text-sm font-medium opacity-40">{title}</p>
                {items.map((link) => (
                  <li key={link}>
                    <p className="py-1.5 text-xs md:text-sm text-txt font-normal transition-colors hover:text-blue-gray-900">{link}</p>
                  </li>
                ))}
              </ul>
            ))}
        ...
        <div className="mt-12 flex w-full flex-col items-center justify-center border-t border-blue-gray-50 py-4 md:flex-row md:justify-between">
          <p className="mb-4 text-center text-sm font-normal text-blue-gray-900 md:mb-0">
            &copy; {currentYear} <a>Browse our relatives</a>. All Rights Reserved.
          </p>
